...
public class Trie {
	...
	// 保存创建的单个实例
	private static Trie hashTree = null;
	// 保存上次加载的路径
	 ...
	 * @return None
	 */
	private Trie(String path) throws Exception {
	 ...
	 * @return 哈希树
	 */
	public static Trie getInstance(String path) throws Exception {
		if(hashTree == null) {
			hashTree = new  Trie(path);
			loadPath = path;
		} else {
	 ...
	 * @return 匹配的结束位置
	 */
	public Word matchParOfStr(String str, int startPos) {
		int len = str.length();
		Map<Character, Node> tmpMap = root;
		Node tmpNode = null;
